...
public static class DumpExtensions
{
    ...
    {
        object getValueType(object value, Type type)
        {
            return (value, type) switch
            {
                ...
                (_, Type t) when t.IsArray && t.GetElementType() == typeof(byte) => (value as IEnumerable).Cast<byte>()
                    .Select(b => $"{b:X2}").ToArray(),
                (object v, _) => v,
    ...
    public static void AsYaml(this object value)
    {
        AsYaml(value, Console.WriteLine);
    }
